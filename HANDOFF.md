...
- Ticket **10 social previews & launch**: code half done this session, deploy half pending (HITL). Details in the ticket's Progress section.
...
2. **Cloudflare Pages (HITL)** — Workers & Pages → Create → Pages → Connect to Git → repo `alekssandar30/portfolio`, production branch `main`:
   - Build command `pnpm build`, output directory `dist`, root `/`.
   - Env var (production + preview): `SITE_URL=https://aleksandar-novakovic.dev`. Build image v2 picks pnpm 10 from `packageManager`; lockfile is v9 so `--frozen-lockfile` is fine.
   - Custom domains: add `aleksandar-novakovic.dev` and `www.aleksandar-novakovic.dev` (CNAMEs auto-created on the existing zone).
   - Redirect `www` → apex: Rules → Redirect Rules → single redirect, hostname `www.aleksandar-novakovic.dev`, 301 to `https://aleksandar-novakovic.dev${uri}` (or Bulk Redirect). Cloudflare Pages itself does not do this.
   - Optional: Web Analytics → add site, paste the beacon snippet into `BaseLayout.astro` `<head>` (settles the analytics fog item). `_headers` has no CSP, so no allow-listing needed.
